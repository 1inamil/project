...
deleteBtn.addEventListener('click', ()=>{
	...
	Swal.fire({
	  ...
	  preConfirm: () => {
	    return $.ajax({
			url: '/greenpaw/personal_album_delete_ok.do',
			type: 'post',
			data: {
				seq: seq,
				imageList: imageList
		...
		/*fetch(`../personal_album/personal_album_delete_ok.jsp?seq=${seq}`)
	      .then(response => {
	        if (!response.ok) {
	    ...
	    Swal.fire({
	      title: '삭제되었습니다.'
			...
			if (result.isConfirmed) {
				location.href='/greenpaw/personal_album_list.do' //list 페이지로 이동						
				}
	  		})
		}
	})
