...
deleteBtn.addEventListener('click', ()=>{
	...
	Swal.fire({
	  ...
	  preConfirm: () => {
	    return $.ajax({
			url: '/greenpaw/thumb_board/thumb_delete_ok.do',
			type: 'post',
			data: {
				seq: seq,
				imageList: imageList
		...
		/*fetch(`../thumb_board/thumb_delete_ok.jsp?seq=${seq}`)
	      .then(response => {
	        if (!response.ok) {
	    ...
	    Swal.fire({
	      title: '삭제되었습니다.'
			...
			if (result.isConfirmed) {
				location.href='/greenpaw/thumbnail_board.do?categorySeq=5' //list 페이지로 이동						
				}
	  		})
		}
	})
