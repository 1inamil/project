...
function sendImage(img, callBackFunc) {
	...
	$.ajax({
		data: image,
		type: "post",
		url: "/greenpaw/admin_notice/save_img.do",
		contentType: false,
		processData: false,
...
function clickSaveBtn() {
	...
	const finalImg = getFinalSaveImg(markdownContent);
	
	//console.log('-----------');
	const removedImg = tmpImg.filter(x => !finalImg.includes(x));
	...
	if (isTitleEmpty) {
		...
		$.ajax({
			...
			type: "post",
			url: "/greenpaw/admin_notice_write_ok.do",
			dataType: "json",
			success: function (result) {
				...
				if (flag == 1) {
					alert('글쓰기에 성공했습니다.');
					location.href='/greenpaw/admin_noticeboard.do?category='+result.category; //list 페이지로 이동
